<html>
<head>
    <title>Seat Booking-Cinetix</title>
<link rel="stylesheet" href="style/seat.css">
<link rel="icon" href="cinetix.png" type="image/png">
</head>
<body style="background: #3a4660;">
<br>
<div class="plane">
 <div class="cockpit">
    <h1>Please Select Your Seats</h1>
 </div>
<ol class="cabin fuselage">
   <li class="row row--1">
   <ol class="seats" type="A">
<li class="seat">
<label for="1"></label>
</li>
       <li class="seat">
	<label for="1">A</label>
        </li>
<li class="seat">
	<label for="2">B</label>
        </li>
<li class="seat">
	<label for="3">C</label>
        </li>
<li class="seat">
	<label for="4">D</label>
        </li>
<li class="seat">
	<label for="5">E</label>
        </li>
  </ol>


  <li class="row row--1">
	<ol class="seats" type="A">
	<li><label>1</label></li>
	<li class="seat">
        </li>

        <li class="seat">
        <input type="checkbox" id="1A" />
        </li>
	<li class="seat">
        <input type="checkbox" id="1B" />
        </li>
        <li class="seat">
        <input type="checkbox" id="1C" />
        </li>
	<li class="seat">
        <input type="checkbox" disabled id="1D" />
        </li>
	<li class="seat">
        <input type="checkbox" id="1E">
        </li>
      </ol>
    </li>

<li class="row row--2">
	<ol class="seats" type="B">
	<li><label>2</label></li>
	<li class="seat">
        </li>

        <li class="seat">
        <input type="checkbox" id="1A" />
        </li>
	<li class="seat">
        <input type="checkbox" id="1B" />
        </li>
        <li class="seat">
        <input type="checkbox"disabled  id="1C" />
        </li>
	<li class="seat">
        <input type="checkbox" disabled id="1D" />
        </li>
	<li class="seat">
        <input type="checkbox" id="1E">
        </li>
      </ol>
    </li>

<li class="row row--3">
	<ol class="seats" type="C">
	<li><label>3</label></li>
	<li class="seat">
        </li>

        <li class="seat">
        <input type="checkbox" id="1A" />
        </li>
	<li class="seat">
        <input type="checkbox"disabled  id="1B" />
        </li>
        <li class="seat">
        <input type="checkbox" id="1C" />
        </li>
	<li class="seat">
        <input type="checkbox" id="1D" />
        </li>
	<li class="seat">
        <input type="checkbox" id="1E">
        </li>
      </ol>
    </li>
    <li class="row row--4">
	<ol class="seats" type="D">
	<li><label>4</label></li>
	<li class="seat">
        </li>

        <li class="seat">
        <input type="checkbox" id="1A" />
        </li>
	<li class="seat">
        <input type="checkbox" id="1B" />
        </li>
        <li class="seat">
        <input type="checkbox"disabled  id="1C" />
        </li>
	<li class="seat">
        <input type="checkbox" id="1D" />
        </li>
	<li class="seat">
        <input type="checkbox" id="1E">
        </li>
      </ol>
    </li>
    <li class="row row--5">
	<ol class="seats" type="E">
	<li><label>5</label></li>
	<li class="seat">
        </li>

        <li class="seat">
        <input type="checkbox" id="1A" />
        </li>
	<li class="seat">
        <input type="checkbox"disabled  id="1B" />
        </li>
        <li class="seat">
        <input type="checkbox" id="1C" />
        </li>
	<li class="seat">
        <input type="checkbox" id="1D" />
        </li>
	<li class="seat">
        <input type="checkbox" id="1E">
        </li>
      </ol>
    </li>
    <li class="row row--6">
	<ol class="seats" type="F">
	<li><label>6</label></li>
	<li class="seat">
        </li>

        <li class="seat">
        <input type="checkbox" id="1A" />
        </li>
	<li class="seat">
        <input type="checkbox" id="1B" />
        </li>
        <li class="seat">
        <input type="checkbox"disabled  id="1C" />
        </li>
	<li class="seat">
        <input type="checkbox" id="1D" />
        </li>
	<li class="seat">
        <input type="checkbox" id="1E">
        </li>
      </ol>
    </li>
    <li class="row row--7">
	<ol class="seats" type="G">
	<li><label>7</label></li>
	<li class="seat">
        </li>

        <li class="seat">
        <input type="checkbox" id="1A" />
        </li>
	<li class="seat">
        <input type="checkbox" id="1B" />
        </li>
        <li class="seat">
        <input type="checkbox" id="1C" />
        </li>
	<li class="seat">
        <input type="checkbox" id="1D" />
        </li>
	<li class="seat">
        <input type="checkbox" id="1E">
        </li>
      </ol>
    </li>
<li class="row row--8">
	<ol class="seats" type="H">
	<li><label>8</label></li>
	<li class="seat">
        </li>

        <li class="seat">
        <input type="checkbox" disabled id="1A" />
        </li>
	<li class="seat">
        <input type="checkbox" id="1B" />
        </li>
        <li class="seat">
        <input type="checkbox" id="1C" />
        </li>
	<li class="seat">
        <input type="checkbox" id="1D" />
        </li>
	<li class="seat">
        <input type="checkbox" id="1E">
        </li>
      </ol>
    </li>
</ol>
<div class="exit exit--back fuselage">All Eyes Here (screen) </div>
<center><input type="button" value="Next" name="submit" onClick="location.href='homecopy.php'"></center>
</div>
</body>
</html>